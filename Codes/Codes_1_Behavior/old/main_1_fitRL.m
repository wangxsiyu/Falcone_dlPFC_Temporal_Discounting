...
parfor gi = 1:length(games)
    ...
    for modeli = 1:nmodel
        if isempty(xfit{modeli, gi}) || xfit{modeli, gi}.LL < -500 || ... 
                (W.isfield(xfit{modeli, gi}.params_table, 'timeYP') && xfit{modeli, gi}.params_table.timeYP < 0) || ...
                (W.isfield(xfit{modeli, gi}.params_table, 'value_future') && xfit{modeli, gi}.params_table.value_future < 0)
