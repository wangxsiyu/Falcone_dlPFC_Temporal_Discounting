...
%% unused code (do not modify)
% %% process deltaV
% addpath("../Codes_1_Behavior/");
...
%     ttab = unique(animalGames(:, ["condition", "drop", "delay", "cue1"]));
%     ttab.YP = (ttab.cue1 == "yellow") * 1 + (ttab.cue1 == "purple") * 2;
%     ttab = ttab(ttab.YP > 0, :);
%     if ai == 1
%         tmd = fit_overall{6, 1};
%         md = Model3t;
% 
%         [ttab.va, ttab.vr] = md.compVal(W.cellfun(@(x)tmd.(x), md.name_parameters), ttab.drop, ttab.delay, ttab.YP == 1);
%         ttab.dv = ttab.va - ttab.vr;
%     else
%         tmd = fit_overall{6, 2};
%         md = Model3t;
% 
%         [ttab.va, vr] = md.compVal(W.cellfun(@(x)tmd.(x), md.name_parameters), ttab.drop, ttab.delay, ttab.YP == 1);
%         ttab.vr(:,1) = vr;
%         ttab.dv = ttab.va - ttab.vr;
%     end
%     tabs{ai} = ttab;
...
% c0 = W.cellfun_vertcat(@(g)W.cond_average_tab(g, 'condition', {'choice', 'DV_overall'}), games_all);
    ...
    % tid = pval < 0.05 & timeAt < 1000 & timeAt > 0;
    % avY = mean(yellowValue(:, tid), 2)';
    % avP = mean(purpleValue(:, tid), 2)';
    % avDV = avP - avY;
    % tt = tabs{animalIndex};
    % behDV = [tt.va(1:2:end) - tt.va(2:2:end)];
    % plt.scatter(behDV, avDV, 'corr');
    ...
    %     if wi == 2
    %     y = squeeze(bp(windowi,ttid, 1, wi));
    %     plt.scatter(x, y, 'dot', 'color', yellowColor);
    %     y = squeeze(bp(windowi,ttid, 2, wi));
...
%% Regression parameters expressed on the value-axis projection
% This section is intentionally appended after the original Figure_YP code.
% Cells were recorded in different sessions, so a simultaneous trial-level
% population projection is unavailable. Because OLS and projection are
% linear operations, fitting each cell and then calculating v'*beta gives
% the corresponding regression coefficient of the value-axis projection.
regressionValueAxis = W.load('../../TempData/value_axisGO');
regressionAxisWindow = 1;
regressionBaselineWindow = [-250 0];
regressionParameterNames = [ ...
    "Intercept (condition 1)", ...
    "Condition 2 - 1", "Condition 3 - 1", ...
    "Condition 4 - 1", "Condition 5 - 1", ...
    "Condition 6 - 1", "Condition 7 - 1", ...
    "Condition 8 - 1", "Condition 9 - 1", ...
    "Choice", "Release1", "Choice x Release1"];
regressionAnimalColors = [0.15 0.45 0.75; 0.75 0.25 0.20];
valueProjectionRegression = struct;
valueProjectionRegression.parameter_names = regressionParameterNames;
valueProjectionRegression.model = [ ...
    "activity ~ 1 + condition(2:9) + choice + release1 + " ...
    "choice*release1"];

for regressionAnimalIndex = 1:2
    regressionData = go{regressionAnimalIndex};
    regressionTime = regressionData.time_at;
    nRegressionNeurons = numel(regressionData.cells);
    nRegressionTimes = numel(regressionTime);
    nRegressionParameters = numel(regressionParameterNames);
    regressionBetas = nan(nRegressionNeurons, ...
        nRegressionParameters, nRegressionTimes);

    for regressionNeuronIndex = 1:nRegressionNeurons
        regressionSpikes = ...
            regressionData.cells{regressionNeuronIndex};
        regressionGame = regressionData.games{ ...
            regressionData.info_cells.gameID(regressionNeuronIndex)};

        conditionTerms = zeros(height(regressionGame), 8);
        for regressionCondition = 2:9
            conditionTerms(:, regressionCondition - 1) = ...
                regressionGame.condition == regressionCondition;
        end
        choiceCode = 2 * regressionGame.choice - 1;
        releaseCode = 2 * regressionGame.release1 - 1;
        regressionDesign = [ ...
            ones(height(regressionGame), 1), ...
            conditionTerms, choiceCode, releaseCode, ...
            choiceCode .* releaseCode];
        validBehavior = all(isfinite(regressionDesign), 2);

        for regressionTimeIndex = 1:nRegressionTimes
            validTrials = validBehavior & ...
                isfinite(regressionSpikes(:, regressionTimeIndex));
            currentDesign = regressionDesign(validTrials, :);
            if rank(currentDesign) < nRegressionParameters
                continue;
            end
            regressionBetas(regressionNeuronIndex, :, ...
                regressionTimeIndex) = ...
                currentDesign \ regressionSpikes( ...
                validTrials, regressionTimeIndex);
        end
    end

    regressionBaselineMask = ...
        regressionTime >= regressionBaselineWindow(1) & ...
        regressionTime < regressionBaselineWindow(2);
    regressionBetas = regressionBetas - mean( ...
        regressionBetas(:, :, regressionBaselineMask), ...
        3, 'omitnan');

    regressionAxis = regressionValueAxis.animals( ...
        regressionAnimalIndex).axis_unit(:, regressionAxisWindow);
    validRegressionNeurons = all(reshape( ...
        isfinite(regressionBetas), nRegressionNeurons, []), 2) & ...
        isfinite(regressionAxis);
    regressionBetas(~validRegressionNeurons, :, :) = 0;
    regressionAxis(~validRegressionNeurons) = 0;
    regressionAxis = regressionAxis / norm(regressionAxis);

    parameterProjection = reshape(regressionAxis' * ...
        reshape(regressionBetas, nRegressionNeurons, []), ...
        nRegressionParameters, nRegressionTimes);
    valueProjectionRegression.animals( ...
        regressionAnimalIndex).name = ...
        animalNames(regressionAnimalIndex);
    valueProjectionRegression.animals( ...
        regressionAnimalIndex).time_at = regressionTime;
    valueProjectionRegression.animals( ...
        regressionAnimalIndex).parameter_projection = ...
        parameterProjection;
    valueProjectionRegression.animals( ...
        regressionAnimalIndex).n_valid_neurons = ...
        sum(validRegressionNeurons);
end

figure('Color', 'w', ...
    'Name', 'Regression of the value-axis projection', ...
    'Position', [100 100 1400 800]);
tiledlayout(3, 4, 'TileSpacing', 'compact', 'Padding', 'compact');
for regressionParameterIndex = 1:nRegressionParameters
    nexttile;
    hold on;
    for regressionAnimalIndex = 1:2
        regressionResult = valueProjectionRegression.animals( ...
            regressionAnimalIndex);
        regressionTime = regressionResult.time_at;
        smoothingBins = max(1, round( ...
            50 / median(diff(regressionTime))));
        parameterTimeCourse = smoothdata( ...
            regressionResult.parameter_projection( ...
            regressionParameterIndex, :), ...
            'gaussian', smoothingBins);
        plot(regressionTime, parameterTimeCourse, ...
            'Color', regressionAnimalColors( ...
            regressionAnimalIndex, :), ...
            'LineWidth', 1.6);
    end
    ...
    xlim([-250 1000]);
    set(gca, 'XTick', -250:250:1000);
    title(regressionParameterNames( ...
        regressionParameterIndex), 'Interpreter', 'none');
    box off;
    set(gca, 'FontSize', 10);
    if regressionParameterIndex > 8
        xlabel('Time from GO onset (ms)');
    end
    if mod(regressionParameterIndex - 1, 4) == 0
        ylabel('Coefficient on value axis');
    end
    if regressionParameterIndex == 1
        legend(cellstr(animalNames), 'Location', 'best', ...
            'Box', 'off');
    end
end
sgtitle({'Regression of value-axis projection', ...
    'Coefficients baseline-corrected to -250:0 ms'});
