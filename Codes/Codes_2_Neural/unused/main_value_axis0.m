%% Estimate the pre-GO Model 1 value-coding axis
trainingWindow = [-250 0];
factorNames = {'DV_overall'};
animalNames = ["Monkey S", "Monkey T"];

valueAxis = struct;
valueAxis.settings = struct( ...
    'training_lock', 'GO cue onset', ...
    'training_window_ms', trainingWindow, ...
    'value_definition', 'Model 1 DV_overall', ...
    'coefficient_method', ...
    'W.anovan_slidingwindow on condition-averaged activity', ...
    'model', 'firing rate ~ 1 + standardized DV_overall', ...
...
for animalIndex = 1:numel(animalNames)
    ...
    valueAxis.animals(animalIndex).axis_unit = betaValue/axisNorm;
end

W.save('../../TempData/value_axis', 'valueAxis', valueAxis);
