function plot_dprojections(plt, projections, games)
%PLOT_DPROJECTIONS Plot group-difference trajectories for each condition.
% YP options show yellow minus purple. Motor options show release minus
% hold. Each panel therefore contains at most nine DV-colored curves.

    trial_options = { ...
        'YP', 'motor', 'motor_controlYP', 'YP_controlMotor'};
    option_is_yp = [true, false, false, true];
    ...
    for trial_option_index = 1:numel(trial_options)
        ...
        if option_is_yp(trial_option_index)
            difference_order = [1 2];
            difference_label = 'Yellow - purple projection';
        else
            difference_order = [2 1];
            difference_label = 'Release - hold projection';
        end
            ...
            for axis_index = 1:n_axes
                ...
                for row_index = 1:n_rows
                    ...
                    for rank_index = 1:n_conditions
                        ...
                        first_trajectory = reshape(values(row_index, ...
                            condition, difference_order(1), :), 1, n_times);
                        second_trajectory = reshape(values(row_index, ...
                            condition, difference_order(2), :), 1, n_times);
                        difference_trajectory = ...
                            first_trajectory - second_trajectory;
                        if any(~isfinite(difference_trajectory))
                            continue;
                        end
                        plot(time_at, difference_trajectory, ...
                            'Color', offer_colors( ...
        ...
        plt.update(['dprojections_' trial_option]);
    end
end
