function plot_projections_YP_RH(plt, projections)
...
% during -250 to 0 ms before the mean and SE are taken across conditions.
    ...
    for trial_option_index = 1:numel(trial_options)
        ...
        if option_is_yp(trial_option_index)
            ...
            if strcmp(trial_option, 'YP')
                group_names = {'Yellow', 'Purple'};
            else
                group_names = { ...
                    'Yellow (motor matched)', ...
                    'Purple (motor matched)'};
            end
        else
            group_colors = {hold_color, release_color};
            if strcmp(trial_option, 'motor')
        ...
        plt.update(['projections_YP_RH_' trial_option]);
    end
end
